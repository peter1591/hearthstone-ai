...
namespace mcts
{
	namespace selection
	{
		...
		public:
			...
			bool CheckBoard(judge::view::BoardView const& new_view) {
				std::lock_guard<Utils::SpinLock> lock(mutex_);
		...
		private:
			bool LockedCheckBoard(judge::view::BoardView const& new_view) {
				if (!board_view_) {
					board_view_.reset(new judge::view::BoardView(new_view));
					return true;
				}
		...
		private:
			...
			std::unique_ptr<judge::view::BoardView> board_view_;
