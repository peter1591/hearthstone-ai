...
namespace engine {
	...
	public:
		...
		void ForEachMinion(state::PlayerSide view_side, Functor && functor) const {
			GetPlayer(view_side).minions_.ForEach([&](state::CardRef card_ref) {
				...
				bool attackable = FlowControl::ValidActionGetter(state_).IsAttackable(card);
		...
		bool IsHeroAttackable(state::PlayerSide view_side) const {
			return FlowControl::ValidActionGetter(state_).IsAttackable(
	...
	public:
		...
		auto GetValidActionGetter() const { return FlowControl::ValidActionGetter(state_); }
