...
namespace FlowControl
{
	namespace Helpers
	{
		...
		public:
			...
			{
				const Entity::Card & first_hero = state.mgr.Get(state.board.players.Get(State::kPlayerFirst).hero_ref_);
				bool first_died = first_hero.GetHP() < 0;
				const Entity::Card & second_hero = state.mgr.Get(state.board.players.Get(State::kPlayerSecond).hero_ref_);
