	...
	namespace Manipulators
	{
		namespace Helpers
		{
			...
			public:
				...
				OrderedCardsManager(state::State & state, state::FlowContext & flow_context, ContainerType & container)
					: state_(state), container_(container), flow_context_(flow_context) {}

				static OrderedCardsManager FromDeck(state::State & state, state::FlowContext & flow_context, state::PlayerIdentifier player);
