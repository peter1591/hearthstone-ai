...
namespace GameEngine {
	...
	public:
		...
		static SlotIndex GetOppositeSide(SlotIndex side)
		{
			if (side == SLOT_PLAYER_SIDE) return SLOT_OPPONENT_SIDE;
			else if (side == SLOT_OPPONENT_SIDE) return SLOT_PLAYER_SIDE;
			else throw std::runtime_error("invalid argument");
		}
		...
		enum SideTargetType {
			...
			TARGET_TYPE_MINIONS_TARGETABLE_BY_FRIENDLY_SPELL,
			TARGET_TYPE_CHARACTERS_TARGETABLE_BY_FRIENDLY_SPELL,
		...
		enum TargetType {
			...
			TARGET_SPELL_ENEMY_MINIONS,
			TARGET_SPELL_ENEMY_CHARACTERS,
			TARGET_SPELL_FRIENDLY_MINIONS,
			TARGET_SPELL_FRIENDLY_CHARACTERS
		};
